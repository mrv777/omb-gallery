...
export default function HolderDistributionHistogram({
  buckets,
}: {
  buckets: HolderDistributionBucketRow[];
}) {
  const lookup = new Map(buckets.map(b => [b.bucket, b.wallet_count]));
    ...
    <ChartFrame title="holder distribution" subtitle="wallets by # held">
      ...
      >
        ...
        {counts.map((c, i) => {
          ...
          return (
            <g key={BUCKETS[i]}>
              <rect x={x} y={y} width={barW} height={h} fill="#ff8a2a" />
