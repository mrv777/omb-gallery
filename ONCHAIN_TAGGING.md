...
- **Adjacent: estimating expiration of an active loan.** We _can_ tag an active loan via the §2.4 origination fingerprint (it's a tx-level property — that _did_ happen, on chain), and we _can_ observe that the resulting escrow hasn't been spent yet. What we can't extract from chain is the specific CSV value baked into the escrow's tap-tree, since that's hidden behind the merkle root (above). Liquidium offers a discrete menu of term lengths (empirically 7/10/12/14/16/30d), and lender vaults specialize on one term most of the time, so an estimate of the form "this loan will probably expire on {origination + vault's modal term}" is high-coverage and easy to caveat. Code lives in `src/lib/loanExpiration.ts`; the corpus is `loan-defaulted` events whose default-leaf CSV decoded successfully. **This is explicitly a heuristic UX feature, not a chain-fingerprint.** The estimate confidence is exposed alongside it (`estimated_basis: 'vault'|'global'|'unknown'`, `estimated_sample_count: N`) and the UI labels it "Estimate only — actual term not visible on chain." Don't promote this to `events.raw_json` or to any DB column that downstream code might mistake for ground truth — it stays a derived/computed value, refreshed in-process every 5 min, and never persisted as if it were chain data.
...
- **Escrow / borrower:** the escrow is **not** derived from tx structure here — the caller uses the OMB's ord-detected destination (`events.new_owner`). The borrower/buyer is unrecoverable on chain (the principal pays the _seller_, and `old_owner` is the seller, not the borrower; the escrow is opaque until spend per §2.3), so `effective_owner` anchors to the escrow and self-corrects to the real holder when the loan resolves (§2.5).
...
The original fee-address rule below remains the fallback for legacy settlements. Modern offer acceptance and bulk listing settlements use the additional executed-script rules in §2.11.1. They are not cooperative-only.
...
### 2.11.1 Modern ord.net offers and bulk listing settlements (2026-09-13)

**Promoted chain fingerprints.** Shared implementation: `scripts/lib/ord-net-settlement.js`, imported by both `src/lib/ordNetFingerprintTick.ts` and the historical script. No API is called by these detectors. APIs were used only to cross-check public sale records against confirmed bitcoind transactions.

**Offer acceptance v2:** the executed Taproot leaf starts with `OP_FALSE OP_IF "ordnet-offer/v2"`, an 8-byte little-endian gross offer price, expiry, buyer destination script, and two commitments. The executable suffix checks a SHA256 preimage, a one-of-two buyer/service signature, and two mandatory signer keys. The service key is `d2dc3222298e2a5f4e1c7d702fae2bcf7821cc0a095a478b95c62195b0df7398`; the mandatory signers are `8efe604eb9dfa01d33404656dafa2aefea83660f01fa04ae0686a6110957b86f` and `3e4cb29671c3b25fa0b23d902cd46102d1c158420e8e00daac552d23b013a7d7`. Match the entire executed leaf and acceptance witness, not a substring or an unexecuted metadata marker.

The supported shape has 3 inputs and 5 outputs. Input 0 is the event's old owner's inscription, delivered with unchanged postage to output 0 and the event's new owner. Output 0 must exactly match the buyer script committed in the offer. Input 1 must fund exactly the committed price; the revealed preimage and control-block key must match the script. Input 2's 333-sat anchor is regenerated at output 4. The seller DEFAULT signature commits the transaction, including output 1's payment (at least 50,000 sats, no greater than gross price). A seller's BTC payment address can differ from their ordinals address. The optional split at output 3 and the anchor address can vary; they are not brand-identification markers.

**Evidence:** 30 confirmed offer transactions cross-checked against the latest 200 public OMB sale entries on September 13. Every script-derived gross price equals the published price, including #60570592 (1,810,000 sats), #60580804 (1,805,000 sats), and #83311107 (1,515,000 sats). Complete minimal chain fixtures are in `tests/chain-fixtures/ordnet-settlements.json`; representative cases are registered in `scripts/known-transactions.json`. These calls consumed confirmed, node-validated transactions; the matcher does not independently re-run Bitcoin signature validation.

**Bulk listing sales:** require the established ord.net fee output and at least two executed 2-of-2 listing leaves `<seller pubkey> CHECKSIG <service pubkey> CHECKSIGADD 2 NUMEQUAL`. Witness slot 0 holds the service DEFAULT signature; slot 1 holds the seller's 65-byte `SINGLE|ANYONECANPAY` signature. (The older generic detector examined slot 0 only.) Input N commits to a payment of at least 50,000 sats at output N. Use FIFO sat ranges to prove that the event's entire postage output comes from exactly one of these signed listing inputs, with matching destination owner. This avoids relying on seller-address uniqueness or how many event rows arrived in a tick.

Evidence covers five inscription sales in two independent bulk transactions: `c502b854…` (#83308279, #60579038) and `b7d8a3e8…` (#83307313, #83308651, #83313908). The latter includes repeated sellers and identical postage/payment amounts. A signed payout proves each sale, but gross asking price cannot be recovered without assumptions about fees. These events therefore become `sold` with `sale_price_sats=NULL`; sale counts and alerts work, and BTC volume stays unchanged. The legacy no-price/no-payment safeguard remains intact for transactions without these executed listing proofs. Existing already-sold prices are preserved.

**Counterexamples and regression checks:** confirmed transfer `42f328df…`, loan `973fe9df…`, and delivery leg `0e039849…`; mutated service keys, missing signatures, changed metadata, wrong preimage/buyer/funding, missing payment, mismatched inscription destinations, bad control blocks, unsafe integers, and ambiguous FIFO ranges. Unit tests exercise every fixture. SQLite-backed live-tick and actual CLI tests verify idempotency, aggregates, RPC-failure retry, split-tick bulk arrival, and historical notification suppression.

**Repair:** run `scripts/backfill-ord-net-fingerprint.js --settlements-only --dry-run --verbose` before applying `--settlements-only`. The script and its shared `lib/ord-net-settlement.js` must be staged together. It only upgrades existing untagged transfers or tags existing untagged sales; it never inserts new events, rewinds live cursors, or enqueues historical notifications. RPC failures produce a nonzero exit status. A schema migration is not required.
