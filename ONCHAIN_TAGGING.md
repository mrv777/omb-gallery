...
- **Adjacent: estimating expiration of an active loan.** We *can* tag an active loan via the §2.4 origination fingerprint (it's a tx-level property — that *did* happen, on chain), and we *can* observe that the resulting escrow hasn't been spent yet. What we can't extract from chain is the specific CSV value baked into the escrow's tap-tree, since that's hidden behind the merkle root (above). Liquidium offers a discrete menu of term lengths (empirically 7/10/12/14/16/30d), and lender vaults specialize on one term most of the time, so an estimate of the form "this loan will probably expire on {origination + vault's modal term}" is high-coverage and easy to caveat. Code lives in `src/lib/loanExpiration.ts`; the corpus is `loan-defaulted` events whose default-leaf CSV decoded successfully. **This is explicitly a heuristic UX feature, not a chain-fingerprint.** The estimate confidence is exposed alongside it (`estimated_basis: 'vault'|'global'|'unknown'`, `estimated_sample_count: N`) and the UI labels it "Estimate only — actual term not visible on chain." Don't promote this to `events.raw_json` or to any DB column that downstream code might mistake for ground truth — it stays a derived/computed value, refreshed in-process every 5 min, and never persisted as if it were chain data.
